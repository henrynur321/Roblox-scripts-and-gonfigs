...
--[[
    ...
    he literally fell asleep in vr while testing lol
    
    feel the power :smiling_imp:
...
cons.fly = renderstepped:Connect(function(deltaTime)
    if flymultiplier < -0.01 then
        cam.CFrame += (racf * handsOffset).LookVector * flymultiplier * deltaTime
    end
end)
flyspeed = -flyspeed
local deadzone = 0.05
cons.ic = uis.InputChanged:Connect(function(key)
    if key.KeyCode == flybutton then
        key = key.Position.Z
        if key < deadzone then
            key = 0
        else
            key = key - deadzone
        end
        flymultiplier = key * flyspeed
    end
end)
